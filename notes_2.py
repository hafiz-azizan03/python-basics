...
#                   Good for I/O bound tasksIlike reading files or fetching data from APIs
...
#chore1 = threading. Thread(target=walk_dog, args=("Scooby", "Doo"))  #the args are tuples, so if we only have 1 tuple, then we would write ("Scooby",)
#chore1. start ()
...
#chore1. join()  #the join() method is used to ensure that the main program waits for each thread to complete its execution before moving on to the next part of the code.
#chore2. join()
#chore3. join()
